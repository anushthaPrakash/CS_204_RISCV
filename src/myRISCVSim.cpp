...
static int stall = 0, stop = 0;int cycle_no = 1;int Fetchflag = 0;int endflag = 0;int rflag = 0;int newflag = 0;
int inst_end = 1, T_inst=0, Ld_St = 0, ALU_inst = 0, Control_inst = 0, Branch_Mis = 0;
float CPI;
        ...
        if (subtype == "beq"){
            if (X[operand1] == X[operand2]){
                if (BrPr[DE_EX.progcont]==0) { 
                    pc -= 4;
                    BrPr[DE_EX.progcont]=1; Fetchflag=2;
                    ch='T';
                    ...
                    // pc += imm-4; already done in decode 
                }           
            }
            else{
                ch='F';
