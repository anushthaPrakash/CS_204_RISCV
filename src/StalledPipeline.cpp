...
int cycle_no = 1;int Fetchflag = 0;int endflag = 0;int rflag = 0;int newflag = 0;
...
void load_register()
{
    ...
    for (i = 0; i < 32; i++)  {fprintf(fp, "x%u - %u                                                                                 \n", i, X[i]);}
    fclose(fp);
}
...
void load_dhazards() {
    ...
    for (i = 0; i < siz; i++)   {   fprintf(fp, "Cycle number: %d\n", dhcycle[i]);}
    fclose(fp);
}
...
void fetch()
{
    cycle_no++;     cout<<"Fetch Instruction:\n";
    ...
    else if ((Tall > 0 || stall == 0) && Fetchflag!=2)
    {
        pc += 4; 
        Tall = 0;
    ...
    else if (Fetchflag==2 && EX_MA.check=='T') {Fetchflag=0; endflag=0; cout<<"Branch Stall\n";}
    else if (Fetchflag==2 && EX_MA.check=='F') {Fetchflag=0; endflag=0; cout<<"Branch Stall\n"; pc-=4;}
    else {cout<<"Stalled.\n";}
    cout<<"EX_MA.check="<<EX_MA.check<<"\nendflag="<<endflag<<"\nFetchflag="<<Fetchflag<<endl;
}
    ...
    if ((stall+newflag)==0) 
    {
        ...
        switch (Type)
        {
            ...
            case 'I':
            {
                ...
                if (operand1 == DE_EX.rd2 && cycle_no > 2 && DE_EX.rd2 != 0 && subtype!="jalr")       {stall = 4;Tall = 4; Stalls_D+=3; Data_H++; dhcycle.push_back(cycle_no-1);}
                else if (operand1 == MA_WB.rd2 && cycle_no > 4 && MA_WB.rd2 != 0 && subtype!="jalr")    {stall = 3;Tall = 3; Stalls_D+=2; Data_H++; dhcycle.push_back(cycle_no-1);}
                else if ((operand1 == WBB.rd2)&& cycle_no > 5 && WBB.rd2 != 0 && subtype!="jalr")    {stall = 2; Tall = 2; Stalls_D+=1; Data_H++; dhcycle.push_back(cycle_no-1);}
                break;
            }
            case 'S':
            {
                ...
                if ((operand1 == DE_EX.rd2 || operand2 == DE_EX.rd2) && DE_EX.rd2 != 0)      { stall = 4; Tall = 4; Stalls_D+=3; Data_H++; dhcycle.push_back(cycle_no-1);}
                else if (operand1 == MA_WB.rd2 || operand2 == MA_WB.rd2 && MA_WB.rd2 != 0)    {stall = 3; Tall = 3; Stalls_D+=2; Data_H++; dhcycle.push_back(cycle_no-1);}
                else if ((operand1 == WBB.rd2 || operand2 == WBB.rd2)&& WBB.rd2 != 0)    {stall = 2; Tall = 2; Stalls_D+=1; Data_H++; dhcycle.push_back(cycle_no-1);}
                break;
            }
            case 'B':
            {
                ...
                else if ((operand1 == WBB.rd2 || operand2 == WBB.rd2)&& WBB.rd2 != 0)    {stall = 2; Tall = 2; MA_WB.Stall = stall-1; Stalls_D+=1; Data_H++; dhcycle.push_back(cycle_no-1);}
                break;
            }
            case 'U':
            {
...
void mem()
{
    ...
    if (stall>0) {
        MA_WB.Stall = --stall;
        cout << "Stalled.\n";
        EX_MA.rs1 = 0; EX_MA.rs2 = 0; EX_MA.rd2 = 0; EX_MA.rd_value = 0; EX_MA.type = '0'; EX_MA.Subtype = "";
    }
...
void write_back()
{
    ...
    if (stall>0) {
        cout<<"Stalled.\n";
        MA_WB.rd_value = 0; MA_WB.rd2 = 0; MA_WB.Subtype = ""; MA_WB.type='0';
    }
    else{
...
void run_riscvsim()
{
    ...
    {
        int STALL = 0;
        if (DE_EX.Stall>0) STALL=DE_EX.Stall-1;
        if (inst_end > 0){
